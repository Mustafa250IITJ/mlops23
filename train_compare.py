...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python filename.py <model_filename.joblib>")
        sys.exit(1)
    ...
    # production_model = train_production_model(X_train, y_train)
    ...
    # compare_models(production_model, candidate_model, X_test, y_test)
    ...
    if "production_model" in model_filename:
        ...
        # model = train_production_model(X_train, y_train)
        # prod_filename = model_filename
        # joblib.dump(model, prod_filename)
        model = train_candidate_model(X_train, y_train)
        joblib.dump(model, model_filename)
